...
public class C45Split
  ...
  private void handleNumericAttribute(Instances trainInstances)
	...
	break;
      ...
      i++;
	  newdistribution.add(instance.value(m_attIndex));
    }
    firstMiss = i;
	
	
	simplemin= attmean(newdistribution); //Anis cherfi
    ...
    defaultEnt = infoGainCrit.oldEnt(m_distribution);
		 ...
		 while ((j < firstMiss) && (trainInstances.instance(j).value(m_attIndex) <= simplemin))
		{
			j++;
		}
		
								
							m_distribution.shiftRange(1,0,trainInstances,0,j);
  ...
  public static double attmean(ArrayList<Double> m) {
   double sum = 0;
